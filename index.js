...
if (!isRegistered) return reply(nad.noregis())
if (isLimit(sender)) return reply(nad.limitend(pusname))
if (args.length < 1) return reply(`「❗」Contoh : ${prefix}hartatahta Farel`)
har = body.slice(12)
...
case 'cloudtext':
				...
				if (!isRegistered) return reply(nad.noregis())
				if (!isPrem) return reply(nad.premium())
if (args.length < 1) return reply(`「❗」Contoh : ${prefix}cloudtext Farel`)
cloud = body.slice(11)
...
*/
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							baby.sendMessage(_.jid, buff, image, {caption: `*「 BLODY BROADCAST 」*\n\n${body.slice(4)}`})
						}
						reply('')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `*「 DISCLAIMER 」*\n\n${body.slice(4)}`)
						}
						reply('*「 SUKSES BOSKU 」*')
					}
					break
